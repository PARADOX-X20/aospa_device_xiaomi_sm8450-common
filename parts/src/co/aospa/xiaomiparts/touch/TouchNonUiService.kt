            ...
            override fun onReceive(context: Context, intent: Intent) {
                ...
                when (intent.action) {
                    ...
                    Intent.ACTION_SCREEN_OFF -> {
                        ...
                        // Check for the standard AOSP setting for DT2W
                        val aospDoubleTapEnabled = Settings.Secure.getInt(
                                contentResolver,
                                Settings.Secure.DOUBLE_TAP_TO_WAKE, 0
                            ) > 0

                        // Check if Screen-Off UDFPS is enabled
                        val udfpsEnabled =
                            ambientConfig.screenOffUdfpsEnabled(UserHandle.myUserId())
                        
                        dlog(
                            TAG,
                            "aospDoubleTapEnabled=$aospDoubleTapEnabled udfpsEnabled=$udfpsEnabled"
                        )

                        // Activate pocket mode if either DT2W or UDFPS is enabled
                        if (aospDoubleTapEnabled || udfpsEnabled) {
